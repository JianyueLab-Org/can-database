...
机场详情页列出这个机场有哪些图、图号、图种、多大 —— **但不给链接**。那 3.3 GB 的 PDF 目前
在集群里没有地方放（can-db 的 AGENTS.md 里写了三条路和为什么），所以每一个链接点下去都会
是 404。**一个必然 404 的链接比没有链接糟**，和 can-efb 那四个占位页是同一条。

等 PDF 有地方放了，这一段加 `href` 就行 —— 索引里 `filePath` 已经在了。在那之前不要把它
渲染成 `<a>`。
