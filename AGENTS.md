...
can-db 那边的搜索挑（每条程序是一条候选边，不是按代号排序取第一条），这里只显示。**规划在 can-db**（`internal/aip/route.go`），理由和总览页那两个统计一样：第二个消
...
- **两种命中要看得出区别。** 每条限制带一个 `scope`：`segment` 是本次真的飞到了发布的那
  一段，`airway` 是只有航路对上（汇编里「W13航线…」说的是整条航路，却只挂在其中几段上，
  所以两种都得列）。两个不同颜色的标签加一句解释，**不要合并成一种** —— 合并等于告诉看
  的人这两者一样确定。
