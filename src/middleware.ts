...
import { ACCESS_READ, signInUrl } from "@/lib/config";
 ...
 * 范围。
 *
 * 门槛是 `aipAccess >= 1`，也就是 ADM 在 can-api 的 `user` 表上授予的那一列。它
 * 和评级（rating）是两条独立的轴 —— 一个校对航图的人可能一个管制评级都没有，一
 * 个 ADM 也未必该编辑程序编码。
 *
 * ## 这一次，「便利不是边界」这句话要反过来读
 *
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  if (user.aipAccess < ACCESS_READ && pathname !== "/denied") {
