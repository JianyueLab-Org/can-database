 ...
 * 一张表旁边的导出按钮。
 *
 * **它不渲染表格。** 八张表里四张是 Astro 直出的 HTML，为了导出把它们改写成 Vue
 * 是把渲染搬到运行时去换一个按钮，不值得 —— 所以这个岛屿只接数据、只管出文件。
 *
 * 行数组因此会在那四页的 HTML 里出现两次（一次成表格，一次作 props）。四张表都是
 * 单场规模，可以接受；全库量级只出现在数据库里，不出现在任何一页上。
 */
